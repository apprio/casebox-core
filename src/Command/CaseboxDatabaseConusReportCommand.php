 ...
 (SELECT COUNT(*) FROM objects where
  ...
  (SELECT COUNT(*) FROM objects where
   ...
   (SELECT COUNT(*) FROM objects where
      sys_data like \'%"case_status":"Cerrado"%\'
			and (je(data, \'_location_type\') in (LOCATION_STUFF) or je(data, \'_location_type.value\') in (LOCATION_STUFF))
      and sys_data like \'%"closurereason_s":"Cliente se ha mudado fuera del �rea de servicio%\' and
      ...
      sys_data like \'%"case_status":"Cerrado"%\'
         ...
         and dstatus = 0
			...
			and name not like \'%-  []%\' and dstatus = 0 AND DATE(tree.cdate) = \''.$date.'\') referrals_food,  
			 (select count(*) from tree where template_id =607 and name like \'Muebles y Electrodom�sticos%\'
			AND tree.pid in (select id from objects 
            where (je(data, \'_location_type\') in (LOCATION_STUFF) or je(data, \'_location_type.value\') in (LOCATION_STUFF)))
...
and IF(template_id=141,id,pid) in (select id from objects 
	...
	$ohseprsql = 'select  
			...
			SUM(IF (sys_data like \'%espec�fico para el tipo de dano%\',1,0) ) home_hazard_insurance,
			...
			SUM(IF (sys_data like \'%"employmentreferralneeded_s":"S�"%\',1,0) ) employment_referral_needed,
			SUM(IF (sys_data like \'%"healthinsurancelostdisaster_s":"S�"%\',1,0) ) insurance_lost_to_disaster,
			SUM(IF (sys_data like \'%"healthhavehealthinsurance_s":"S�"%\',1,0) ) insurance_have_insurance,
			SUM(IF (sys_data like \'%"healthinsurancetype_s":"S-Chip%"%\',1,0) ) insurance_s_chip,
			SUM(IF (sys_data like \'%"transportationreferralneeded_s":"S�"%\',1,0) ) transportation_referral_needed,
			SUM(IF (sys_data like \'%"medicalreferralneeded_s":"S�"%\',1,0) ) health_referral_needed,
            SUM(IF (sys_data like \'%"medicalliketospeak_s":"S�"%\',1,0) ) client_speak_to_someone,
            SUM(IF (sys_data like \'%"medicalindistress_s":"S�"%\',1,0) ) relational_stress,			
			SUM(IF (sys_data like \'%Cuidado de Ninos%\' AND sys_data like \'%"childassesmentreferralneeded_s"%\',1,0) ) child_care_referral_needed,
			SUM(IF (sys_data like \'%"childassesmentfosterchildren_s":"S�"%\',1,0) ) child_fostercare,
			...
			SUM(IF (sys_data like \'%"foodreferralneeded_s":"S�"%\',1,0) ) food_referral_needed,
			SUM(IF (sys_data like \'%D-SNAP%\',1,0) ) dsnap_referral_needed,
			SUM(IF (sys_data like \'%"clothingreferralneeded_s":"S�"%\',1,0) ) clothing_referral_needed,
			SUM(IF (sys_data like \'%"furnitureandappliancesreferralneeded_s":"S�"%\',1,0) ) furniture_referral_needed,
			SUM(IF (sys_data like \'%"seniorservicesreferralneeded_s":"S�"%\',1,0) ) senior_referral_needed,
			SUM(IF (sys_data like \'%"languagereferralneeded_s":"S�"%\',1,0) ) language_referral_needed,
			SUM(IF (sys_data like \'%"legalservicesreferralneeded_s":"S�"%\',1,0) ) legal_referral_needed,
			(SELECT SUBSTR(tree.name, 1, LOCATE(\'-\', tree.name) - 1) FROM   objects, tree where objects.id = tree.id 
			AND tree.template_id = 607 and tree.name not like \'%-  []%\' and dstatus = 0 
			AND (DATE(tree.cdate) = \''.$date.'\')
			AND tree.pid in (select id from objects 
